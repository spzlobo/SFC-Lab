# A Simple SFC example with multiple SF
...
```bash
echo -e "template_name: ssh-firewall-vnfd
description: SSH firewall

service_properties:
  Id: ssh-firewall-vnfd
  vendor: tacker
  version: 1
  type:
      - firewall
  ...
  vdu1:
    ...
    user_data: |
      ...
      runcmd:
        - [ curl, "https://raw.githubusercontent.com/opendaylight/sfc/master/sfc-test/nsh-tools/vxlan_tool.py", -sLO, /usr/bin/vxlan_tool.py ]
        - [ chmod, +x, /usr/bin/vxlan_tool.py ]
        - [ python3, /usr/bin/vxlan_tool.py, -i eth0, -d forward, -v off, -b 22 ]

      ssh_authorized_keys:
        - \"$(cat ~/.ssh/sfc_demo.pub)\"
config:
  param0: key0
  param1: key1" > ssh-firewall-vnfd.yaml
```
...
tacker vnfd-create --vnfd-file ./ssh-firewall-vnfd.yaml
tacker vnfd-list
```
...
```bash
tacker sfc-create --name firewall-chain --chain http-firewall,ssh-firewall
tacker sfc-list
```
...
tacker device-template-delete ssh-firewall-vnfd
